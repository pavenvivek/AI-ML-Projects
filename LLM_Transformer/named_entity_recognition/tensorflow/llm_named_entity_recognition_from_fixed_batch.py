...
def pad_and_trim_batch(inputs):
    ...
    if length < SEQ_LEN:
        ...
        inputs = tf.concat([inputs, tf.zeros((padding_size,), dtype=inputs.dtype)], axis=0)
    else:
        inputs = tf.slice(inputs, [0], [SEQ_LEN])
...
train_dataset = (
    ...
    .map(lambda x, y: (pad_and_trim_batch(x), pad_and_trim_batch(y)))
